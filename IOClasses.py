# ---------------------------------------------------------- #
# Title: IOClasses (copied from Listing 11)
...
class EmployeeIO:
    ...
    def input_menu_options():
        ...
        while(True):
            ...
            print()  # Add an extra line for looks
            try:    #added try except function in case of user input error
                choice = int(choice)
                ...
                print(str(choice) + " is not a valid number (1 - 4).")
    ...
    def print_current_list_items(list_of_rows: list):
        ...
        for row in list_of_rows:
            print(str(DC.Employee(row.employee_id, row.first_name, row.last_name).employee_id) 
                  + "," 
                  + DC.Employee(row.employee_id, row.first_name, row.last_name).first_name  
                  + "," 
                  + DC.Employee(row.employee_id, row.first_name, row.last_name).last_name)
        print("************************************************")
        print()  # Add an extra line for looks
